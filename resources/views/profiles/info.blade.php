...
@section('content')
    <div class="container-fluid p-0">
        ...
        <ul class="nav nav-tabs">
            <li class="nav-item">
                <a class="nav-link" href="{{ route('profiles.about', ['name' => $other_user->name]) }}"><i class="fas fa-portrait"></i> {{ __('Info') }}</a>
            </li>
            <li class="nav-item">
            ...
            @if ($other_user->id == $user->id)
                <li class="nav-item">
                    <a class="nav-link active" href="{{ route('profiles.info', ['name' => $other_user->name]) }}"><i class="fas fa-info-circle"></i> {{ __('Info') }} ({{ \App\Models\ModeratorAction::where('user_id', $user->id)->where('moderator_only', 0)->count() }})</a>
                </li>
            @endif
        </ul>

                        ...
                        @if (\Illuminate\Support\Facades\Auth::Id() != $follow->id)
                            @if ($user->name == $other_user->name)
                                @if ( $user->friends($follow->id) )
                                    ...
                                    @endif
                                @else
                                    <button type="button" class="btn btn-link" data-id="{{ $follow->id }}" data-html="true" data-toggle="tooltip" data-title="Użytkownik <b>{{ $follow->name }}</b> nie obserwuje Cie, wiec nie mozesz udostepnic mu swoich ukrytych prac."  data-delay='{"show":"500", "hide":"300"}'><i class="far fa-eye-slash" style="height: 30px; width: 30px; color: #4e555b; opacity: 25%"></i></button>
                                @endif
